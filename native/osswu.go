...
// Sswu4Params for computing the Simplified SWU mapping
...
func (p *Sswu6Params) Osswu3mod4(u *Field6) (x, y *Field6) {
	var tv1, tv2, tv3, tv4, xd, x1n, x2n, gxd, gx1, aNeg, zA, y1, y2 [Field6Limbs]uint64
	var wasInverted int
	u.Arithmetic.Mul(&tv1, &u.Value, &u.Value) // tv1 = u^2
	u.Arithmetic.Mul(&tv3, &p.Z, &tv1)         // tv3 = z * tv1
	u.Arithmetic.Square(&tv2, &tv3)            // tv2 = tv3^2
	...
	Pow6(&y1, &tv4, &p.C1, u.Params, u.Arithmetic) // y1 = tv4^C1
	u.Arithmetic.Mul(&y1, &y1, &tv2)               // y1 = y1 * tv2
	u.Arithmetic.Mul(&x2n, &tv3, &x1n)             // x2n = tv3 * x1n
	...
	e2 := (&Field6{Value: tv2}).Equal(&Field6{Value: gx1})
